...
public class TestModel {
    ...
    public static void main(String[] args) {
        ...
        // Captured variables
        IntVar[] sequence = model.intVarArray("sequence", seq_length, 0, 3);
        ...
        // Ports' statuses
        BoolVar[] service_status = model.boolVarArray("service_status", seq_length + 1); //model.intVarArray("service_status", seq_length + 1, 0, 1);
        for(int i = 0; i < seq_length + 1; i++) {
...
//        model.arithm(states[seq_length], "=", deployed).post(); // inferred
        ...
        model.arithm(service_status[seq_length], "=", disabled).post(); // inferred
        IntVar count_service_disabled = model.intVar("count_service_disabled", 0, seq_length); // inferred
        model.sum(service_status, "=", count_service_disabled).post(); // inferred
        model.arithm(count_service_disabled, ">", 0).post(); // inferred
...
//            System.out.println("Minimal cost: " + scost.getValue());
//            System.out.println("Places: "); Arrays.stream(states).forEach(System.out::print);
//            System.out.println("Sequence: "); Arrays.stream(sequence).forEach(System.out::print);
        } else {
            solver.reset();
...
// ARITHM ([states[12] = 2]),
// ARITHM ([service_status[12] = 1]),
// ARITHM ([prop(REIF_47.EQ.REIF_48)]),
// REIF_48 = [0,1]=>ARITHM ([states[12] = 2]),
// !REIF_48 = [0,1]=>ARITHM ([states[12] =/= 2]),
// REIF_47 = [0,1]=>ARITHM ([service_status[12] = 0]),
// !REIF_47 = [0,1]=>ARITHM ([service_status[12] =/= 0])