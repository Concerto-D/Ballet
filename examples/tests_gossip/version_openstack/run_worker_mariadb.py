...
common_worker = Common(versions=versions)
common_worker.set_name(f"commonworker{i}")
haproxy_worker = Haproxy(versions=versions)
haproxy_worker.set_name(f"haproxyworker{i}")
memcached_worker = Memcached(versions=versions)
memcached_worker.set_name(f"memcachedsworker{i}")
ovswitch_worker = Ovswitch(versions=versions)
ovswitch_worker.set_name(f"ovswitchworker{i}")
rabbitmq_worker = Rabbitmq(versions=versions)
rabbitmq_worker.set_name(f"rabbitmqworker{i}")
facts_worker = Facts(versions=versions)
facts_worker.set_name(f"factsworker{i}")
keystone_worker = Keystone(versions=versions)
keystone_worker.set_name(f"keystoneworker{i}")
glance_worker = Glance(versions=versions)
...
for version in versions:
    connections.append(('mariadbmaster',f'servicev{version}',f'mariadbworker{i}',f'masterservicev{version}'))  
    connections.append((f'factsworker{i}',f'servicev{version}',f'commonworker{i}',f'factsservicev{version}'))
    connections.append((f'factsworker{i}',f'servicev{version}',f'haproxyworker{i}',f'factsservicev{version}'))
    connections.append((f'factsworker{i}',f'servicev{version}',f'memcachedworker{i}',f'factsservicev{version}'))
    connections.append((f'factsworker{i}',f'servicev{version}',f'ovswitchworker{i}',f'factsservicev{version}'))
    connections.append((f'factsworker{i}',f'servicev{version}',f'rabbitmqworker{i}',f'factsservicev{version}'))
    connections.append((f'commonworker{i}',f'servicev{version}',f'mariadbworker{i}',f'commonservicev{version}'))
    connections.append((f'haproxyworker{i}',f'servicev{version}',f'mariadbworker{i}',f'haproxyservice'))
    connections.append((f'mariadbworker{i}',f'servicev{version}', f'keystoneworker{i}', f'mariadbservicev{version}'))  
    connections.append((f'mariadbworker{i}',f'servicev{version}', f'glanceworker{i}', f'mariadbservicev{version}'))  
    connections.append((f'mariadbworker{i}',f'servicev{version}', f'novaworker{i}', f'mariadbservicev{version}'))  
    connections.append((f'mariadbworker{i}',f'servicev{version}', f'neutronworker{i}', f'mariadbservicev{version}'))  
    connections.append((f'keystoneworker{i}',f'servicev{version}', f'novaworker{i}', f'keystoneservicev{version}'))  
...
active = {
    ...
    common_worker : 'deployedv1',
    haproxy_worker : 'deployedv1',
    memcached_worker : 'deployedv1',
    ovswitch_worker : 'deployedv1',
    rabbitmq_worker : 'deployedv1',
    facts_worker : 'deployedv1',
    keystone_worker : 'deployedv1',
    glance_worker : 'deployedv1'
}
